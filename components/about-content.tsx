            ...
            {teamMembers.map((member, index) => (
              ...
              >
                <div className="relative w-48 h-48 mx-auto mb-6 rounded-full overflow-hidden border-4 border-[#F3ECFF]">
                  <Image src={member.image || "/placeholder.svg"} alt={`${member.name} - ${member.role} at Route to Recall`} fill className="object-cover" />
                </div>
                <h3 className="text-xl font-bold mb-1 text-[#1C1C1C]">{member.name}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {companyValues.map((value, index) => (
              ...
              >
                ...
                <h3 className="text-2xl font-bold mb-4 text-[#1C1C1C]">{value.title}</h3>
