...
export async function POST(request: NextRequest) {
  try {
    ...
    const htmlContent = getEmailHtml("contact", { name, email, phone, subject, message })
    ...
    if (!res.ok) {
      console.error("[v0] Brevo contact error", res.status, responseText)
    ...
    console.error("[v0] contact route error:", error?.message)
    return NextResponse.json(
